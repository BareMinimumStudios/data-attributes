...
class AttributeFunctionHeaderComponent(override var identifier: Identifier, private val functions: MutableMap<Identifier, AttributeFunction>, private val provider: AttributeFunctionProvider, private val options: UIAttributeComponentOptions) : CollapsibleFoldableContainer(Sizing.content(), Sizing.content(), Text.of("<n/a>"), DataAttributesClient.UI_STATE.collapsible.functionParents[identifier.toString()] ?: true), AttributeConfigComponent<EntityAttribute> {
    override val registry: Registry<EntityAttribute>
        get() = Registries.ATTRIBUTE

    private fun createEntry(childId: Identifier, function: AttributeFunction): AttributeFunctionComponent = childById(AttributeFunctionComponent::class.java, "child#$childId") ?: AttributeFunctionComponent(childId, function, identifier, provider, options).also { it.id("child#$childId") }.also(::child)
    ...
    private fun addFunctions(functions: MutableMap<Identifier, AttributeFunction>) {
            ...
            child(
                ...
                { _, _ ->
                    ...
                    Components.button(Text.translatable("text.config.data_attributes.buttons.add")) {
                        ...
                        child(2, AttributeFunctionComponent(childId, function, identifier, provider, options))
